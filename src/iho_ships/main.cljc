(ns iho_ships.main
  "Kotodama WASM entrypoint for the iho_ships clean-room actor (L4) — Clojure port.
  ...
  facts (`iho_ships.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iho_ships")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"iho_ships.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iho_ships-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iho_ships-compat/ready)
